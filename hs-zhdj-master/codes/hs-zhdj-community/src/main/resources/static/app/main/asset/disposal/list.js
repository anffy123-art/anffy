(function ($app) {
    $app.module('gtPartyApp').controller('assetDisposalListCtrl', function ($scope, $http, $modal, $ocLazyLoad, $alert, $loading, $state, $bsRouterState, AuthUser, $timeout, $excel) {
        ...
        $scope.getList = function () {
            $http
            ({
                method: 'get', url: '../../api/asset/getDisposalList', params: $scope.selectparams
        ...
        $scope.add = function () {
            $bsRouterState.$closeTabName("home.assetDisposalAdd");
            $timeout(function () {
                $bsRouterState.go('home.assetDisposalAdd', {
                    id: ""
                });
            }, 100);
        };
        $scope.edit = function (id) {
            $bsRouterState.$closeTabName("home.assetDisposalEdit");
            $timeout(function () {
                $bsRouterState.go('home.assetDisposalEdit', {
                    id: id
                });
            }, 100);
        };
        $scope.show = function (id,assetid) {
            $bsRouterState.$closeTabName("home.assetDisposalShow");
            $timeout(function () {
                $bsRouterState.go('home.assetDisposalShow', {id: id,assetid:assetid});
            }, 100);
        };
        ...
        $scope.$on('tabChildReloadData', function (event, data) {
            if (data.name == 'assetDisposalListCtrl') {
                $scope.getList();
            }
        });
