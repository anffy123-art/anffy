(function ($app) {
    $app.module('gtPartyApp').controller('assetMaintainEditCtrl', function ($scope, $http, $modal, $ocLazyLoad, $alert, $loading, $state, $bsRouterState, AuthUser, $timeout, $partySelect) {
        ...
        $scope.getData = function () {
            $http.get("../../api/asset/getMaintainById", {
                ...
                if (!$scope.dataModel.applyuserid) {
                    $scope.dataModel.applyuserid = AuthUser.getUser().Id;
                    $scope.dataModel.applyusername = AuthUser.getUser().Name;
                }
        ...
        $scope.submitData = function (isflag) {
            ...
            if (isflag) {
                if (confirm("确认提交？")) {
                    ...
                    $http({
                        method: "post",
                        url: "../../api/asset/saveMaintain",
                        data: $scope.dataModel
                    }).then(function (result) {
                        $loading.hide();
                        if (result.data.success) {
                            $scope.showMsg('成功', result.data.msg);
                            $scope.$emit("tabReloadData", {name: 'assetMaintainListCtrl', data: 1});
                            $scope.pagechange();
                        } else {
        ...
        $scope.$watch('selectApplyuser.selectedValueData', function (newVal) {
            if (newVal != '' && newVal != undefined) {
                $scope.dataModel.applyuserid = $scope.selectApplyuser.selectedValueData;
                $scope.dataModel.applyusername = $scope.selectApplyuser.selectedLabeData;
            }
        });
