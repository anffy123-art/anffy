...
pv.Format.date = function (pattern) {
    ...
    format.parse = function (s) {
        ...
        if (match) match.forEach(function (m, i) {
            fields[i](m);
        });
            ...
            function floor(d, p) {
                switch (p) {
                    ...
                    case 2592e6:
                        d.setDate(1);
                    case 6048e5:
                        if (p == 6048e5) d.setDate(d.getDate() - d.getDay());
                    case 864e5:
                        d.setHours(0);
                    case 36e5:
                        d.setMinutes(0);
...
pv.SvgScene.area = function (scenes) {
    ...
    for (var i = 0; i < scenes.length; i++) {
        si = scenes[i];
        if (!si.width && !si.height) continue;
...
pv.Layout.Matrix = function () {
    ...
    this.label
        ...
        .left(function () {
            return this.index & 1 ? dx * ((this.index >> 1) + .5) : null;
        })
        .top(function () {
            return this.index & 1 ? null : dy * ((this.index >> 1) + .5);
        })
        .textMargin(4)
        .textAlign(function () {
