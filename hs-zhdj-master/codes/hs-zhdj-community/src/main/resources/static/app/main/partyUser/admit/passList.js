(function ($app) {
    $app.module('gtPartyApp').controller('admitPassCtrl', function ($scope, $http, $modal, $ocLazyLoad, $alert, $loading, $state, $bsRouterState, AuthUser, $timeout, $excel,$filter) {
        $state.params = $bsRouterState.$params($scope);
        $scope.loginUserId = AuthUser.getUser().Id;
        $scope.userStatus = 6;//发展对象
        $scope.upUserStatus = 5;//积极分子
        $scope.pageId = "admitPassList";
        ...
        $scope.edit = function (userCode) {
            partyAdmitUser_editModal.params = {
                userCode: userCode,
                title: userCode ? "修改发展对象信息" : "新增发展对象信息",
                userStatus: $scope.userStatus,
        ...
        $scope.excelConfig = {
            ...
            readonly: false,
            templateUrl: '/doc/template/发展对象导入模板.xlsx'
        };
