(function ($app) {
    $app.module('gtPartyApp').controller('admitPrepareCtrl', function ($scope, $http, $modal, $ocLazyLoad, $alert, $loading, $state, $bsRouterState, AuthUser, $timeout, $excel,$filter) {
        $state.params = $bsRouterState.$params($scope);
        $scope.loginUserId = AuthUser.getUser().Id;
        $scope.userStatus = 2;//预备党员
        $scope.upUserStatus = 6;//发展对象
        $scope.pageId = "admitPrepareList";
        ...
        $scope.edit = function (userCode) {
            partyAdmitUser_editModal.params = {
                userCode: userCode,
                title: userCode ? "修改预备党员信息" : "新增预备党员信息",
                userStatus: $scope.userStatus,
        ...
        $scope.excelInPartyConfig = {
            ...
            readonly: false,
            templateUrl: '/doc/template/预备党员导入模板.xlsx'
        };

        $scope.importInParty = function () {
            $excel.addFile($scope.excelInPartyConfig).then(function (items) {
                $scope.import(items, 2);
            });
        };
