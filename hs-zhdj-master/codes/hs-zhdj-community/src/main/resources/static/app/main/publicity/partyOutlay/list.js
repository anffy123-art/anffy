(function ($app) {
    $app.module('gtPartyApp').controller('partyOutlayListCtrl', function ($scope, $http, $modal, $ocLazyLoad, $alert, $loading, $state, $bsRouterState, AuthUser, $timeout) {
        ...
        $scope.selectparams = {
            ...
            paytype:$state.params.dtype==undefined?null:$state.params.dtype,
        };
        $scope.resetparams = {
            isunfold: false,//组织结构树展开收缩
            'pageindex': 1,
        ...
        $scope.$on('tabChildReloadData',function (event,data) {
            if(data.name=='partyOutlayListCtrl'){
                $scope.loadData();
            }
        });
        ...
        $scope.loadData = function () {
            $http
            ({
                method: 'get', url: '../../api/publicity/getPartyOutlayList', params: $scope.selectparams
        ...
        $scope.addData = function () {
            $bsRouterState.$closeTabName("home.partyOutlayAdd");
            $timeout(function () {
                $bsRouterState.go('home.partyOutlayAdd', {
                    id: "",
        ...
        $scope.editData = function (id) {
            $bsRouterState.$closeTabName("home.partyOutlayEdit");
            $timeout(function () {
                $bsRouterState.go('home.partyOutlayEdit', {
                    id: id,
                    dzzdm: $scope.selectparams.partyCode
                });
            }, 100);
        ...
        $scope.showData = function (id) {
            $bsRouterState.$closeTabName("home.partyOutlayShow");
            $timeout(function () {
                $bsRouterState.go('home.partyOutlayShow', {id: id, op: 0});
            }, 100);
        };
        ...
        $scope.deleteData = function (id) {
            if (confirm("是否确定删除？")) {
                $http({
                    url: "../../api/publicity/deletePartyOutlayById",
                    method: 'GET',
                    params: {id: id}
                }).then(function (result) {
        ...
        $scope.exportData = function () {
            if ($scope.selectparams.dzzdm != "" && $scope.selectparams.dzzdm != AuthUser.getUser().gddwdm) {
                ...
                window.open('../../api/publicity/exportPartyOutlay' + params);
            }
        };
        ...
        $scope.reset  = function () {
            ...
            $scope.selectparams.paytype=$scope.resetparams.paytype;
